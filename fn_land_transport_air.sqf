...
//Script Settings
	...
	_maxDigit = ATC_maxDigit;
		...
		if (_type isEqualTo 2) exitwith {//Land base player has listed location for delivery
				...
				{
					_cursor = _cursor + 1;
					call compile format ["Land_RequestAirPickup_Pax%1 = player addAction ['WATCHDOG > %1 x Passenger', airboss_fnc_land_transport_air, [3,%1,%2,%3,%4], 7, false, true];",_cursor,_pickup,_delivery,(count units group player)];
		...
		if (_type isEqualTo 3) exitwith {//Land base player has advised number of passengers
				...
				Land_RequestAirPickup_Cancel = player addAction ["WATCHDOG > Cancel Air Transport Request", airboss_fnc_land_transport_air, [4], 19, false, true];

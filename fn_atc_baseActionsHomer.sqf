	ATC_TransferToFlyco = player addAction ["HOMER > Transfer to FLYCO", airboss_fnc_atc_controller, [2], 20, false, true];
	ATC_TransferToWatchdog = player addAction ["HOMER > Transfer to WATCHDOG", airboss_fnc_land_controller, [1], 19, false, true];
	//if (!ATC_onTask) then {
	...
	if (_targets > 0) then {
		ATC_Homer_TrgVector = player addAction ["HOMER > Request Target Vector", airboss_fnc_atc_controller, [10], 17, false, true];
		ATC_Homer_TrgCancel = player addAction ["HOMER > Cancel Training Target", airboss_fnc_atc_controller, [9], 17, false, true];
	};
	_baseNo = 0;
	ATC_Action_VectorBase = [];
	{
		_baseName = BaseNames select _baseNo;
		_action = player addAction [format["HOMER > Request Vector to Base %1",_baseName], airboss_fnc_atc_position_base,[_baseNo],15,false];
		_baseNo = _baseNo + 1;
